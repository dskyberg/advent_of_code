...
#[derive(Debug, Error)]
pub enum Error {
    #[error("Failed to parse str to Robot")]
    BadRobot,
...
impl Robot {
    pub fn move_pos(&mut self, max: &Point) {
        self.pos = self.pos.add_with_wrap(&self.velocity, max);
    }
}

impl From<(Point, Point)> for Robot {
    fn from((pos, velocity): (Point, Point)) -> Self {
        Self { pos, velocity }
    }
}
