...
impl Machine {
    pub fn solve2(&self) -> Option<isize> {
        let x1 = self.a.x;
        let y1 = self.a.y;
        let x2 = self.b.x;
        let y2 = self.b.y;
...
impl TryFrom<((i64, i64), (i64, i64), (i64, i64))> for Machine {
    type Error = String;
    fn try_from(value: ((i64, i64), (i64, i64), (i64, i64))) -> Result<Self, Self::Error> {
        Ok(Machine {
            a: Point::from(value.0),
            b: Point::from(value.1),
            prize: Point::from(value.2),
        })
    }
}
...
pub fn read_data(input: &'static str) -> Result<Vec<Machine>> {
    let (_, machines) = parse_machines(input)?;
    Ok(machines)
}

pub static INPUT: &str = r#"Button A: X+16, Y+32
Button B: X+53, Y+11
Prize: X=6788, Y=4716
